    ...
    <div class="bg-white p-8 rounded-lg shadow-inner border">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" step="0.1" name="profundidad_semilla" id="profundidad_semilla" value="{{ old('profundidad_semilla', $cultivo->profundidad_semilla) }}" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">
            </div>
            <div>
                <label for="cantidad_de_plantas" class="block font-medium text-sm text-gray-700">Cantidad de Plantas</label>
                <input type="number" name="cantidad_de_plantas" id="cantidad_de_plantas" value="{{ old('cantidad_de_plantas', $cultivo->cantidad_de_plantas) }}" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">
            </div>
            <div>
                <label for="costo" class="block font-medium text-sm text-gray-700">Costo Estimado</label>
                <input type="number" step="0.01" name="costo" id="costo" value="{{ old('costo', $cultivo->costo) }}" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">
            </div>
            <div class="flex items-center pt-6">
                <input type="checkbox" name="iluminacion" id="iluminacion" value="1" @checked(old('iluminacion', $cultivo->iluminacion)) class="h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500">
